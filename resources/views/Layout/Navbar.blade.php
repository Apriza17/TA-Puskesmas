...
<body class="bg-gray-100">
    ...
    <div>
        ...
        <div x-data="{ showSuccess: true, showError: true }">
            @if (session('success'))
                <div x-show="showSuccess" x-init="setTimeout(() => showSuccess = false, 4000)" x-transition.duration.500ms
                    class="absolute top-5 right-5 bg-green-200 text-green-800 px-4 py-2 rounded shadow-lg z-50 animate-fade-left ease-in-out duration-200">
                    <div class="flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" class="size-9" width="512" height="512"
                            viewBox="0 0 512 512">
    ...
    <aside
        ...
        <div class="relative border-b-4 border-slate-400 flex items-center justify-between p-3">
            <div class="flex items-center gap-3">
                <img src="img/logo.png" class="w-48" alt="">
                <p class="font-bold text-sky-900 text-xl"></p>
            </div>

            <!-- close button (label yang sama) -->
            <label for="sidebar-toggle"
                class="cursor-pointer text-gray-500 hover:text-gray-700 p-2 rounded-md hover:bg-gray-50">
                <!-- SVG close milikmu -->
        ...
        <nav class="px-3 py-3">
            <ul class="space-y-1 text-gray-700">
                <li>
                    <a href="/dashboard"
                        class="flex items-center py-2 px-4 rounded hover:bg-sky-900 hover:text-white transition {{ Request::is('dashboard') ? 'bg-sky-900 text-white' : '' }}">
                ...
                <li>
                    <a href="/Data-Sartika"
                        class="flex items-center py-2 px-4 rounded hover:bg-sky-900 hover:text-white transition {{ Request::is('Data-Sartika') ? 'bg-sky-900 text-white' : '' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="size-8" viewBox="0 0 24 24">
                            <g fill="none">
                                <path
                ...
                <li class="pt-1">
                    <input type="checkbox" id="menu-laporan" class="peer/menu sr-only" {{ Request::is('Laporan*') ? 'checked' : '' }} />
                    <label for="menu-laporan"
                        class="flex items-center justify-between py-2 px-4 rounded cursor-pointer
                            hover:bg-sky-900 hover:text-white transition {{ Request::is('Laporan*') ? 'bg-sky-900 text-white' : '' }}">
                        ...
                        <svg class="size-5 transition-transform duration-200 peer-checked/menu:rotate-180"
                    ...
                    <ul
                        class="ml-12 mt-1 space-y-1 text-sm max-h-0 overflow-hidden transition-[max-height] duration-300 ease-in-out
                            peer-checked/menu:max-h-48">
                        <li>
                            <a href="/Laporan" class="block px-4 py-1 rounded hover:bg-sky-800 hover:text-white {{ Request::is('Laporan') ? 'bg-sky-800 text-white' : '' }}">Rekap
                                Bulanan Anak</a>
                        </li>
                        <li>
                            <a href="/Laporan-Gizi"
                                class="block px-4 py-1 rounded hover:bg-sky-800 hover:text-white {{ Request::is('Laporan-Gizi') ? 'bg-sky-800 text-white' : '' }}">Rekap Gizi Anak</a>
                        </li>
                        <li>
                            <a href="/Laporan-Sartika"
                                class="block px-4 py-1 rounded hover:bg-sky-800 hover:text-white {{ Request::is('Laporan-Sartika') ? 'bg-sky-800 text-white' : '' }}">Rekap Sartika</a>
                        </li>
                    </ul>
                </li>
                ...
                <li>
                    <a href="/Pengguna"
                        class="flex items-center py-2 px-4 rounded hover:bg-sky-900 hover:text-white transition {{ Request::is('Pengguna') ? 'bg-sky-900 text-white' : '' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="size-8" viewBox="0 0 24 24">
                            <path fill="currentColor" fill-rule="evenodd"
                ...
                <li>
                    <a href="/Pengaturan-admin"
                        class="flex items-center py-2 px-4 rounded hover:bg-sky-900 hover:text-white transition {{ Request::is('Pengaturan-admin') ? 'bg-sky-900 text-white' : '' }}">
                ...
                <li>
                    <a href="{{ url('/Logout') }}"
                        class="flex items-center py-2 px-4 rounded hover:bg-red-800 hover:text-white transition">
                        <svg xmlns="http://www.w3.org/2000/svg" class="size-8" viewBox="0 0 24 24">
                            <path fill="currentColor"
